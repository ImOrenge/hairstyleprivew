...
function snapshot(): MakeupDirectionSnapshot {
  ...
  return {
    ...
    version: 1,
    status: "map_ready",
    source: { faceObservationBundleId: "observation", personalColorProfileId: "profile", selectedStyleId: "style", inputProfileRevision: 1 },
    context: { presentation: "natural_grooming", occasions: ["daily"], preparationMinutes: 10, skillLevel: "basic", finishPreference: "natural", exclusions: [], ownedProductTypes: [], ownedToolTypes: [], gender: null, facialHair: { type: "none", userWantsCoverage: false } },
    modules: MAKEUP_MODULES.map((module) => ({ module, state: "enabled", geometry: { coordinateSpace: "normalized_source_image", anchors: [{ x: 0.5, y: 0.5 }], polygons: [], excludedPolygons: [], vectors: [] }, direction: { enabled: true, intensity: 0.5, colorFamily: null, texture: null, evidenceIds: [], reasons: [], technical: { kind: module, zonePolicyVersion: "makeup-zone-policy-v1", placement: ["zone"], applicationDirection: ["direction"], finish: "natural", technique: "test", productAttributes: [], warnings: [], parameters: {} } } })),
    modelManifest: { geometryPolicyVersion: "v1", directionPolicyVersion: "v1", routinePolicyVersion: "v1", explanationModel: null, createdAt },
    confirmedAt: null,
    createdAt,
  };
}
...
test("makeup rationale proposes but never silently accepts a constrained adjustment", () => {
  const profile = { ...defaultMakeupInterviewProfile(snapshot().context), primaryMode: "full_definition" as const, preparationMinutes: 5 as const, skillLevel: "none" as const, revision: 3 };
  const rationale = compileMakeupRecommendationRationaleV1({ profile, source: snapshot().source, personalColor: { label: "가을 뮤트", confidence: 0.8, palette: ["#8c6d5b"] }, face: { quality: "ready", validSkinPixelRatio: 0.9, warnings: [] }, hair: { colorFamily: "dark brown", fringe: "side", parting: "6:4" } });
...
const observation = {
  schemaVersion: "face-observation-bundle-v2", id: "observation", consultationId: "consultation-v2",
  sourceAnalysisEvidenceId: "evidence", inputHash: "a".repeat(64), modelHash: "b".repeat(64), sourceAssets: [],
  sourceTransform: { rotationDegrees: 0, sourceWidth: 1000, sourceHeight: 1250, coordinateSpace: "normalized-upright-source-v1" },
  landmarks: [], masks: [], calibration: { inputColorSpace: "srgb", workingColorSpace: "linear-srgb", referenceWhite: "D65", method: "srgb-estimated-white-balance-v1", whiteBalanceGains: [1, 1, 1] },
  regionSamples: [{ regionId: "left_cheek_upper", polygon: [{ x: 0.2, y: 0.45 }, { x: 0.42, y: 0.45 }, { x: 0.38, y: 0.68 }], statistics: { median: { l: 70, a: 8, b: 12 }, trimmedMean: { l: 70, a: 8, b: 12 }, mad: { l: 1, a: 1, b: 1 }, chromaMedian: 14, hueDegreesMedian: 56, sampledPixelCount: 100, validPixelCount: 90, validPixelRatio: 0.9 }, excludedByKind: {}, warnings: [] }],
  quality: { status: "usable", validSkinPixelRatio: 0.9, crossRegionMaxDeltaE: null, warnings: [] }, modelManifest: [], correctionRevision: 0, createdAt: "2026-08-15T00:00:00.000Z",
} as FaceObservationBundleV2;
const personalColor = {
  schemaVersion: "personal-color-profile-v2", id: "profile", consultationId: "consultation-v2", version: 1, status: "profile_ready", captureMode: "quick", observationBundleId: "observation",
  calibration: { method: "estimated_white_balance", referenceWhite: "D65", confidence: 0.8, version: "v1", meanDeltaE00: null }, regions: [],
  axes: {
    temperature: { value: 0, confidence: 0.5, evidenceIds: [], unavailableReason: null },
    value: { value: 0, confidence: 0.5, evidenceIds: [], unavailableReason: null },
    chroma: { value: 0, confidence: 0.5, evidenceIds: [], unavailableReason: null },
    contrast: { value: 0, confidence: 0.5, evidenceIds: [], unavailableReason: null },
    hueCharacter: { value: 0, confidence: 0.5, evidenceIds: [], unavailableReason: null },
  },
  seasonalPosterior: [], displayClassification: null, harmonyPalette: { best: ["soft_peach"], base: ["neutral_beige"], accent: ["rose"], challenge: [], metals: [] }, preferenceProfile: { likedColorIds: [], dislikedColorIds: [], preferredContrast: null },
  confidence: { overall: 0.6, typeConfidence: 0.5, paletteConfidence: 0.5, stability: 0.5 }, modelManifest: { profileModel: "v1", axisPolicyVersion: "v1", posteriorVersion: "v1", paletteVersion: "v1", createdAt: "2026-08-15T00:00:00.000Z" }, legacyProjectionHash: null, drapeValidatedAt: null, confirmedAt: null, createdAt: "2026-08-15T00:00:00.000Z",
} as PersonalColorProfileV2;

test("foundation compiler produces seven bounded modules without gender gating", () => {
  const build = (gender: MakeupGender) => buildMakeupFoundationSnapshotV1({
    id: `snapshot-${gender}`, consultationId: "consultation-v2", version: 1,
    source: { faceObservationBundleId: "observation", personalColorProfileId: "profile", selectedStyleId: "style", inputProfileRevision: 1 },
    context: { ...snapshot().context, gender }, observation, personalColor, createdAt: "2026-08-15T00:00:00.000Z",
  });
  ...
  for (const gender of ["male", "female", "nonbinary", "not_provided"] as MakeupGender[]) {
    ...
    assert.deepEqual(candidate.modules.map(({ module, state, direction }) => ({ module, state, enabled: direction.enabled, intensity: direction.intensity })), baseline.modules.map(({ module, state, direction }) => ({ module, state, enabled: direction.enabled, intensity: direction.intensity })));
  }
  ...
  assert.deepEqual(complexionGuides.map((guide) => guide.id), ["t_zone_highlight", "nose_contour", "jaw_shadow"]);
...
test("dense topology projects a deterministic bounded FaceMesh map with seven callouts", () => {
  const denseObservation: FaceObservationBundleV2 = {
    ...
    modelManifest: [{ component: "face-landmarks", provider: "tensorflow-js", name: "MediaPipeFaceMesh", version: "1" }],
  };
  ...
  const built = buildMakeupFoundationSnapshotV1({
    id: "dense-topology", consultationId: "consultation-v2", version: 1,
    source: { faceObservationBundleId: "observation", personalColorProfileId: "profile", selectedStyleId: "style", inputProfileRevision: 1 },
    context: snapshot().context, observation: denseObservation, personalColor, createdAt: "2026-08-15T00:00:00.000Z",
  });
...
test("dense atlas v3 compiles open high-density FaceMesh lines for all three modes", () => {
  const denseObservation: FaceObservationBundleV2 = {
    ...
    modelManifest: [{ component: "face-landmarks", provider: "tensorflow-js", name: "MediaPipeFaceMesh", version: "attention-mesh-1" }],
  };
...
test("dense atlas v3 fails down explicitly instead of fabricating missing landmarks", () => {
  const degraded = compileMakeupDenseAtlasV3({ ...observation, landmarks: Array.from({ length: 467 }, () => ({ x: 0.5, y: 0.5 })) });
...
function semanticFixture(): { atlas: ReturnType<typeof compileMakeupDenseAtlasV3>; artifact: MakeupSemanticArtifactV3 } {
  ...
  const output: MakeupSemanticMapV3 = {
    ...
    zones: [{
      id: "brow.left", module: "brow", purpose: "definition",
      anchorRefs: [70, 63, 105].map((sourceIndex) => ({ lineId: "brow.axis.left" as const, sourceIndex, tangentOffset: 0, normalOffset: 0 })),
      pathMode: "follow_topology", brushDirection: "inner_to_outer", brushStrokeCount: 3, intensity: 0.55,
      reason: "눈썹 결을 따라 자연스러운 선을 만듭니다.", caution: "경계를 진하게 닫지 않습니다.", exclusions: ["hair"], confidence: { semantic: 0.9, visibility: 0.9 },
    }],
    summary: "얼굴의 실제 기준점을 따라 열린 선으로 적용합니다.",
  };
  return { atlas, artifact: { version: "makeup-semantic-artifact-v3", sourceFingerprint: "source-fingerprint", sourceCorrectionRevision: 2, semanticOutputFingerprint: "semantic-fingerprint", output } };
}
...
test("semantic v3 strict output snaps allowlisted anchors and emits open line bundles", () => {
  ...
  const projection = compileMakeupSemanticProjectionV3({ artifact, atlas, expectedSourceFingerprint: "source-fingerprint" });
...
test("semantic v3 rejects protected-region crossings per zone and keeps deterministic fallback", () => {
  ...
  const projection = compileMakeupSemanticProjectionV3({
    ...
    protectedRegions: [{ id: "occlusion", kind: "occluded", points: [{ x: 0.48, y: 0.48 }, { x: 0.52, y: 0.48 }, { x: 0.52, y: 0.52 }, { x: 0.48, y: 0.52 }] }],
  });
...
test("semantic v3 fails closed on stale sources and out-of-contract offsets", () => {
  const { atlas, artifact } = semanticFixture();
  assert.throws(() => compileMakeupSemanticProjectionV3({ artifact, atlas, expectedSourceFingerprint: "changed" }), /SOURCE_STALE/);
  ...
  const extraRoot = { ...structuredClone(artifact.output), inferredAge: "30대" } as unknown as MakeupSemanticMapV3;
  assert.throws(() => assertMakeupSemanticMapV3(extraRoot), /SCHEMA_INVALID/);
  const extraZone = structuredClone(artifact.output) as MakeupSemanticMapV3 & { zones: Array<MakeupSemanticMapV3["zones"][number] & { diagnosis?: string }> };
...
test("P38 fixture matrix covers 30 representation and occlusion combinations within compile budget", () => {
  ...
  for (const fixture of MAKEUP_P38_FIXTURE_CASES) {
    const fixtureObservation = { ...observation, landmarks: makeupP38FixtureLandmarks(fixture) };
    ...
    const built = buildMakeupFoundationSnapshotV1({ id: fixture.id, consultationId: "fixture-consultation", version: 1, source: snapshot().source, context: { ...snapshot().context, gender: fixture.presentationGender }, observation: fixtureObservation, personalColor, createdAt: "2026-08-15T00:00:00.000Z" });
...
test("zone policies provide color position direction intensity texture and product attributes", () => {
  const value = buildMakeupFoundationSnapshotV1({
    id: "zone-snapshot", consultationId: "consultation-v2", version: 1,
    source: { faceObservationBundleId: "observation", personalColorProfileId: "profile", selectedStyleId: "style", inputProfileRevision: 1 },
    context: { ...snapshot().context, preparationMinutes: 20, skillLevel: "intermediate", ownedProductTypes: ["brow_pencil", "lip"] },
    observation, personalColor, hair: { colorFamily: "deep_neutral_brown", fringe: "side", parting: "6:4" }, createdAt: "2026-08-15T00:00:00.000Z",
  });
...
test("time skill presentation facial hair and explicit exclusions alter policy without gender gating", () => {
  const source = { faceObservationBundleId: "observation", personalColorProfileId: "profile", selectedStyleId: "style", inputProfileRevision: 1 };
  const build = (context: MakeupContextProfile, gender: MakeupGender = null) => buildMakeupFoundationSnapshotV1({
    id: `zone-${gender}-${context.presentation}`, consultationId: "consultation-v2", version: 1, source,
    context: { ...context, gender }, observation: { ...observation, masks: [{ id: "facial-hair", label: "facial hair", kind: "facial_hair", operation: "exclude", source: "detected", confidence: 0.9, points: [{ x: 0.35, y: 0.65 }, { x: 0.65, y: 0.65 }, { x: 0.5, y: 0.8 }] }] },
    personalColor, createdAt: "2026-08-15T00:00:00.000Z",
  });
  const natural = build({ ...snapshot().context, preparationMinutes: 5, skillLevel: "none", facialHair: { type: "beard", userWantsCoverage: true }, exclusions: ["no_blush", "glitter"] });
  const expressive = build({ ...snapshot().context, presentation: "expressive", preparationMinutes: 30, skillLevel: "advanced", facialHair: { type: "beard", userWantsCoverage: true }, exclusions: ["no_blush", "glitter"] }, "male");
  ...
  assert.deepEqual(build(natural.context, "female").modules.map((item) => item.module), build(natural.context, "nonbinary").modules.map((item) => item.module));
});

test("manual geometry adjustments reject unsafe deltas and vector magnitude", () => {
  const value = buildMakeupFoundationSnapshotV1({ id: "bounds", consultationId: "consultation-v2", version: 1, source: { faceObservationBundleId: "observation", personalColorProfileId: "profile", selectedStyleId: "style", inputProfileRevision: 1 }, context: snapshot().context, observation, personalColor, createdAt: "2026-08-15T00:00:00.000Z" });
  const brow = value.modules.find((item) => item.module === "brow")!;
  assert.doesNotThrow(() => validateMakeupModulePatchBounds(brow, { anchors: [{ index: 0, point: { x: brow.geometry.anchors[0].x + 0.01, y: brow.geometry.anchors[0].y } }] }));
  assert.throws(() => validateMakeupModulePatchBounds(brow, { anchors: [{ index: 0, point: { x: brow.geometry.anchors[0].x + 0.2, y: brow.geometry.anchors[0].y } }] }), /OUT_OF_BOUNDS/);
  assert.throws(() => validateMakeupModulePatchBounds(brow, { vectors: [{ index: 0, dx: 0.9, dy: 0.9 }] }), /OUT_OF_BOUNDS/);
});

test("confirmed directions compile a bounded routine and an exact structured artist brief", () => {
  const built = buildMakeupFoundationSnapshotV1({ id: "artifact-snapshot", consultationId: "consultation-v2", version: 2, source: { faceObservationBundleId: "observation", personalColorProfileId: "profile-v5", selectedStyleId: "style-v3", inputProfileRevision: 4 }, context: { ...snapshot().context, preparationMinutes: 10, exclusions: ["no_blush"], ownedProductTypes: ["brow_pencil", "lip"] }, observation, personalColor, createdAt: "2026-08-15T00:00:00.000Z" });
  const confirmed: MakeupDirectionSnapshot = { ...built, status: "confirmed", confirmedAt: "2026-08-15T00:05:00.000Z" };
  const routine = compileMakeupRoutineV1({ id: "routine", snapshot: confirmed, createdAt: "2026-08-15T00:05:01.000Z" });
  const brief = compileMakeupArtistBriefV1({ id: "brief", snapshot: confirmed, createdAt: "2026-08-15T00:05:01.000Z" });
  assert.ok(routine.estimatedSeconds <= 600);
  assert.equal(routine.steps.some((step) => step.module === "blush"), false);
