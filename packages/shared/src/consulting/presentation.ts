...
const STAGE_DOMAIN: Record<ConsultationStage, ConsultationDomain> = {
  discovery: "intake", photo: "intake", scan: "hair", analysis: "hair", "personal-color": "color",
  direction: "hair", previews: "hair", compare: "hair", decision: "hair", "color-studio": "color",
  "salon-brief": "hair", makeup: "makeup", fashion: "fashion", result: "report", aftercare: "report",
};

const STAGE_ACTION_LABEL: Record<ConsultationStage, string> = {
  discovery: "상담 목표 정하기", photo: "사진 제출하고 분석 시작", scan: "분석 진행 확인", analysis: "진단 결과 확인",
  "personal-color": "퍼스널 컬러 확인", direction: "추천 방향 확인", previews: "헤어 후보 확인", compare: "후보 비교하기",
  decision: "헤어 스타일 확정", "color-studio": "헤어 컬러 결정", "salon-brief": "Salon Brief 확인",
  makeup: "메이크업 추천 검토", fashion: "패션 방향 정하기", result: "최종 리포트 확인", aftercare: "Aftercare 열기",
};
...
export function deriveConsultationChapterPresentation(
  snapshot: ConsultationSnapshot,
  stage: ConsultationStage = snapshot.currentStage,
): ConsultationChapterPresentationV2 {
  const recommendedStage = snapshot.journey.recommendedStage;
  const recommendedTask = snapshot.journey.activeTasks.find((task) => (
    task.stage === recommendedStage || task.transitionHostStage === recommendedStage
  ));
  ...
  const chapters = CONSULTATION_CHAPTERS.map((id) => {
    ...
    const hasAttention = snapshot.journey.activeTasks.some((task) => (
      consultationChapterForStage(task.stage) === id && task.status === "failed"
    )) || snapshot.journey.blockingActions.some((action) => (
      consultationChapterForStage(action.stage) === id && action.stage === recommendedStage
    ));
    const hasWaiting = snapshot.journey.activeTasks.some((task) => (
      consultationChapterForStage(task.transitionHostStage) === id
      && ["pending", "running", "waiting", "partial"].includes(task.status)
    ...
    const status: ConsultationChapterStatus = hasAttention ? "attention"
      : hasWaiting ? "waiting"
        : activeChapter === id || recommendedChapter === id ? "active"
          : isComplete ? "complete"
            : hasAvailable ? "available" : "locked";
    const availableDomains = (["hair", "color", "makeup", "fashion"] as const).filter((domain) => (
      stages.some((item) => STAGE_DOMAIN[item] === domain && snapshot.journey.allowedStages.includes(item))
    ));
    return { id, status, completedTaskCount, totalTaskCount: stages.length, availableDomains };
  });
...
export function deriveConsultationChapterSurface(
  snapshot: ConsultationSnapshot,
  stage: ConsultationStage = snapshot.currentStage,
): ConsultationChapterSurfaceV1 {
  ...
  const activeTasks = snapshot.journey.activeTasks.filter((task) => (
    task.stage === stage || task.transitionHostStage === stage
  ) && ["pending", "running", "waiting", "partial", "failed"].includes(task.status));
  const artifacts = resultArtifacts(snapshot, stage);
  const visibleQuestions = stage === "analysis"
    ? (snapshot.diagnosticQuestions ?? []).filter((question) => question.state === "visible")
    : [];
  const mode: ConsultationChapterSurfaceMode = blocking || activeTasks.some((task) => task.status === "failed") ? "attention"
    : visibleQuestions.length ? "input"
...
export const CONSULTATION_TASK_MESSAGES: Record<ConsultationTaskKind, readonly string[]> = {
  ...
  "hair-trait-analysis": [
    "사진에서 모발 영역을 얼굴과 배경에서 구분하고 있어요.",
    "결·밀도·볼륨처럼 사진으로 관찰 가능한 특성을 살펴보고 있어요.",
    "사진만으로 알 수 없는 항목은 단정하지 않고 질문으로 남길게요.",
    "기존 답변과 관찰 결과가 충돌하지 않는지 맞추고 있어요.",
  ],
  "personal-color-analysis": [
    "자연광과 색 왜곡이 진단에 영향을 주지 않는지 확인하고 있어요.",
    "온도·명도·채도·대비 축을 함께 비교하고 있어요.",
    "헤어 컬러와 옷에 바로 쓸 수 있는 기준으로 정리할게요.",
  ],
  "preview-generation": [
  ...
  "hair-mask-extraction": [
    "확정한 스타일에서 머리카락 영역을 분리하고 있어요.",
    "잔머리와 얼굴 경계가 자연스럽게 이어지는지 확인하고 있어요.",
    "색상이 피부나 배경으로 번지지 않도록 경계를 다듬고 있어요.",
  ],
  "hair-color-generation": [
    "선택한 색을 확정 헤어의 결에 맞춰 적용하고 있어요.",
    "커트와 얼굴은 유지하고 뿌리·반사광·음영만 다시 만들고 있어요.",
    "피부색과 배경이 바뀌지 않았는지 마지막으로 확인하고 있어요.",
    "고품질 컬러 결과를 저장하고 있어요.",
  ],
  brief: [
  ...
  "result-compilation": [
    "분석과 최종 선택의 근거를 한곳에 모으고 있어요.",
    "퍼스널 컬러와 Salon Brief가 같은 결론을 말하는지 확인하고 있어요.",
    "나중에 다시 열어도 이어질 수 있는 결과를 저장하고 있어요.",
  ],
  "fashion-generation": [
  ...
  "makeup-simulation-generation": [
    "확정한 메이크업 방향과 퍼스널 컬러를 정리하고 있어요.",
    "얼굴·헤어·배경을 유지한 채 메이크업만 적용하고 있어요.",
    "얼굴 형태와 피부 질감이 과도하게 바뀌지 않았는지 확인하고 있어요.",
    "비교할 수 있는 시뮬레이션을 준비하고 있어요.",
  ],
  "aftercare-preparation": [
...
function completedTask(snapshot: ConsultationSnapshot, kind: ConsultationTaskKind, stage: ConsultationStage): ConsultationActiveTask {
  const completedAt = kind === "analysis" ? snapshot.analysisRun?.completedAt
    : kind === "personal-color-analysis" ? snapshot.personalColorDiagnosis.completedAt
    : kind === "brief" ? snapshot.salonBrief.createdAt
      : kind === "result-compilation" ? snapshot.result.compiledAt
        : kind === "hair-color-generation" ? snapshot.colorDecision.confirmedAt
  ...
  const definitions: Record<ConsultationTaskKind, Pick<ConsultationActiveTask, "id" | "originStage" | "transitionHostStage" | "destinationStage" | "readinessKey" | "label" | "completedUnits" | "totalUnits" | "partialOutputCount">> = {
    ...
    "hair-trait-analysis": { id: snapshot.hairTraitAnalysisRun?.id ?? "hair-trait-complete", originStage: "photo", transitionHostStage: "scan", destinationStage: "analysis", readinessKey: "hair-profile-terminal", label: "모질 특성 분석", completedUnits: snapshot.hairProfile?.observed.length ?? 0, totalUnits: 12, partialOutputCount: snapshot.hairProfile?.observed.length ?? 0 },
    "personal-color-analysis": { id: snapshot.personalColorDiagnosis.evidenceId ?? "personal-color-complete", originStage: "analysis", transitionHostStage: "personal-color", destinationStage: "direction", readinessKey: "personal-color-terminal", label: "퍼스널 컬러 분석", completedUnits: 3, totalUnits: 3, partialOutputCount: snapshot.personalColorDiagnosis.state === "ready" ? 1 : 0 },
    ...
    "hair-mask-extraction": { id: snapshot.colorDecision.hairMask?.id ?? "hair-mask-complete", originStage: "decision", transitionHostStage: "color-studio", destinationStage: "color-studio", readinessKey: "hair-mask-ready", label: "헤어 영역 준비", completedUnits: 3, totalUnits: 3, partialOutputCount: snapshot.colorDecision.hairMask ? 1 : 0 },
    "hair-color-generation": { id: snapshot.colorDecision.generationAttemptId ?? "hair-color-complete", originStage: "color-studio", transitionHostStage: "color-studio", destinationStage: "salon-brief", readinessKey: "color-decision-terminal", label: "염색 확정본 생성", completedUnits: 4, totalUnits: 4, partialOutputCount: snapshot.colorDecision.finalImagePath ? 1 : 0 },
    ...
    "result-compilation": { id: snapshot.result.id ?? "result-complete", originStage: "fashion", transitionHostStage: "result", destinationStage: "result", readinessKey: "result-with-fashion-ready", label: "상담 결과 정리", completedUnits: 3, totalUnits: 3, partialOutputCount: snapshot.result.state === "core-ready" || snapshot.result.state === "updated" ? 1 : 0 },
    "fashion-generation": { id: snapshot.fashionBatch?.id ?? "fashion-complete", originStage: "fashion", transitionHostStage: "fashion", destinationStage: "fashion", readinessKey: `fashion-slots-terminal=${snapshot.fashionBatch?.requestedCount ?? 3}`, label: `${snapshot.fashionBatch?.requestedCount ?? 3}개 패션 룩 배치`, completedUnits: snapshot.fashionBatch?.terminalCount ?? 0, totalUnits: snapshot.fashionBatch?.requestedCount ?? 3, partialOutputCount: snapshot.fashionBatch?.completedCount ?? 0 },
    "makeup-simulation-generation": { id: "makeup-simulation-complete", originStage: "makeup", transitionHostStage: "makeup", destinationStage: "makeup", readinessKey: "makeup-simulation-review-ready", label: "메이크업 스타일 시뮬레이션", completedUnits: 1, totalUnits: 1, partialOutputCount: 1 },
    "aftercare-preparation": { id: snapshot.careProgram.actualServiceId ?? "aftercare-complete", originStage: "result", transitionHostStage: "aftercare", destinationStage: "aftercare", readinessKey: "aftercare-program-ready", label: "Aftercare 프로그램", completedUnits: snapshot.careProgram.today.length + snapshot.careProgram.checkpoints.length, totalUnits: snapshot.careProgram.today.length + snapshot.careProgram.checkpoints.length, partialOutputCount: snapshot.careProgram.today.length },
  };
