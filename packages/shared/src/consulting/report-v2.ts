...
export type FaceHairAnalysisSectionV2 = SectionBaseV2<"face-hair-analysis", "hair", {
  distribution: Array<{ label: string; probability: number }>;
  primary: string | null;
  secondary: string | null;
  measurements: Array<{ label: string; value: string; confidence: number | null }>;
  observations: Array<{ label: string; value: string }>;
  confidence: string;
}>;

export type HairDirectionSectionV2 = SectionBaseV2<"hair-direction", "hair", {
  revision: number;
  axes: Array<{ label: string; value: string; reason: string | null; impact: string | null }>;
}>;

export type CandidateComparisonSectionV2 = SectionBaseV2<"candidate-comparison", "hair", {
  candidates: Array<{
    id: string;
    label: string;
    axis: string;
...
export type FinalHairSectionV2 = SectionBaseV2<"final-hair", "hair", {
  selectionId: string;
  label: string;
  image: ConsultationReportImageV2;
  feasibility: string;
...
export type PersonalColorSectionV2 = SectionBaseV2<"personal-color", "color", {
  classification: string | null;
  secondary: string | null;
  posterior: Array<{ label: string; probability: number }>;
  axes: Array<{ key: string; label: string; value: number | null; confidence: number | null }>;
  palettes: { best: string[]; base: string[]; accent: string[]; challenge: string[]; metals: string[] };
  confidence: { capture: number | null; diagnosis: number | null };
}>;

export type FinalColorSectionV2 = SectionBaseV2<"final-color", "color", {
  state: string;
  colorName: string;
  swatchHex: string;
  technique: string;
...
export type MakeupResultSectionV2 = SectionBaseV2<"makeup-result", "makeup", {
...
export type FashionResultSectionV2 = SectionBaseV2<"fashion-result", "fashion", {
  looks: Array<{
    role: string;
    id: string;
    label: string;
...
export type SalonSpecificationSectionV2 = SectionBaseV2<"salon-specification", "final", {
...
export type InitialCareSectionV2 = SectionBaseV2<"initial-care", "final", {
  sourceSelectionId: string | null;
  sourceColorSelectionId: string | null;
  periods: Array<{ label: "24시간" | "첫 3일" | "첫 7일"; actions: string[] }>;
  checklist: string[];
  escalationSigns: string[];
}>;

export type ConsultationReportSectionV2 =
...
function personalColorPosterior(snapshot: ConsultationSnapshot, source: ConsultationReportSourceV2) {
  if (source.personalColorProfile?.seasonalPosterior.length) {
    return source.personalColorProfile.seasonalPosterior.map((item) => ({ label: item.type, probability: item.probability }));
  }
...
function reportImage(
  ...
  alt: string,
  label: string,
  status: ConsultationReportImageV2["status"] = src ? "ready" : "pending",
...
function buildProvenance(
  ...
  source: ConsultationReportSourceV2,
): ConsultingResultProvenanceV3 {
  const hair = source.hairRecommendation?.primaryPreviewId && source.hairRecommendation.confirmedRevision
    ? {
    ...
    : null;
  const fashionSelectedId = source.fashionBatch?.selectedPreviewId
    ?? source.fashionPreviewSet?.selectedStylingSessionId
    ?? snapshot.fashion.lookId;
  const fashionRecommendedId = source.fashionBatch?.recommendedPreviewId ?? fashionSelectedId;
  const fashion = source.fashionBatch && fashionSelectedId && fashionRecommendedId
    ? {
  ...
  const sourceIds = [
    ...
    ...hair?.generatedPreviewIds ?? [],
    ...fashion?.generatedPreviewIds ?? [],
    ...fashion?.productOfferSnapshotIds ?? [],
...
function buildInitialCare(snapshot: ConsultationSnapshot): InitialCareSectionV2 | null {
  ...
  const periods: InitialCareSectionV2["payload"]["periods"] = [
    { label: "24시간", actions: ["두피와 모발에 강한 마찰을 주지 마세요.", "과도한 열기구와 꽉 묶는 스타일을 피하세요."] },
    { label: "첫 3일", actions: ["미지근한 물과 순한 제품으로 부드럽게 세정하세요.", "젖은 모발을 비비지 말고 눌러 물기를 제거한 뒤 완전히 말리세요."] },
    { label: "첫 7일", actions: ["수영장·사우나·강한 자외선 노출을 줄이세요.", "모발 끝과 손상 부위 중심으로 보습 상태를 확인하세요."] },
  ];
  if (colorApplied) {
...
export function projectConsultationReportV2(
  snapshot: ConsultationSnapshot,
  source: ConsultationReportSourceV2 = {},
  profile: ConsultationReportProfileV2 = "full_journey",
  ...
  if (analysisPrimary || snapshot.evidence.items.length || source.analysisEvidence) {
    ...
    sections.push({
      ...
      avoid: [], cautions: [...(source.analysisEvidence ? [] : ["정밀 얼굴형 분포가 없어 확인 가능한 관찰만 표시합니다."]), ...(hairUnknown.length ? [`사진으로 확인할 수 없는 모질 ${hairUnknown.length}개 항목은 미확인으로 유지합니다.`] : [])],
      detailHref: stageHref(snapshot.sessionId, "analysis"),
      payload: {
        distribution, primary: analysisPrimary, secondary: analysisSecondary,
        measurements: (source.analysisEvidence?.measurements ?? []).map((item) => ({ label: item.explanation || item.category, value: `${Math.round(item.normalizedValue * 100) / 100}`, confidence: item.confidence })),
        observations: [
          ["균형", present(snapshot.faceAnalysis.balance)], ["헤어라인", present(snapshot.faceAnalysis.hairline)], ["모량", present(snapshot.faceAnalysis.density)],
        ].filter((item): item is [string, string] => Boolean(item[1])).map(([label, value]) => ({ label, value })).concat(hairObservations),
  ...
  if (snapshot.strategy.confirmedAt || snapshot.strategyRecommendations.length) {
    const axisEntries = [
      ["length", "기장", snapshot.strategy.length], ["fringe", "앞머리", snapshot.strategy.fringe], ["parting", "가르마", snapshot.strategy.parting],
      ["layerStart", "레이어", snapshot.strategy.layerStart], ["crownVolume", "정수리 볼륨", snapshot.strategy.crownVolume], ["sideVolume", "측면 볼륨", snapshot.strategy.sideVolume],
    ...
    sections.push({
      ...
      payload: { revision: snapshot.strategy.revision, axes: axisEntries.map(([key, label, value]) => { const recommendation = snapshot.strategyRecommendations.find((item) => item.axis === key); return { label, value, reason: recommendation?.reason ?? null, impact: recommendation?.impact ?? null }; }) },
    });
  }
    ...
    const candidates: CandidateComparisonSectionV2["payload"]["candidates"] = snapshot.previews.map((item, index) => {
      ...
      return {
        ...
        image: reportImage(
          ...
          `${item.label} 헤어 생성 결과`,
          `${index + 1}. ${item.label}`,
          item.status === "failed" ? "failed" : item.imageUrl ? "ready" : "pending",
        ),
      };
    });
  ...
  if (personalColorStarted) {
    ...
    sections.push({
      ...
      detailHref: stageHref(snapshot.sessionId, "personal-color"), payload: {
        ...
        posterior: personalColorPosterior(snapshot, source), axes,
        palettes: profile?.harmonyPalette ?? { best: snapshot.personalColorDiagnosis.palette.best, base: snapshot.personalColorDiagnosis.palette.neutrals, accent: snapshot.personalColorDiagnosis.palette.accents, challenge: snapshot.personalColorDiagnosis.palette.caution, metals: snapshot.personalColorDiagnosis.palette.metals },
        confidence: { capture: snapshot.personalColorDiagnosis.qualityConfidence, diagnosis: profile?.confidence.overall ?? snapshot.personalColorDiagnosis.qualityConfidence },
      },
    });
  }
  ...
  if (fashionSelected) {
    ...
    const looks: FashionResultSectionV2["payload"]["looks"] = candidates.map((item, index) => ({
      ...
      image: reportImage(
        item.stylingSessionId,
        item.imageUrl,
        `${item.headline} 패션 생성 결과`,
    ...
    if (!looks.length) {
      looks.push({
        role: "selected", id: selectedId, label: fashionSelected.label, category: fashionSelected.category ?? "FINAL", palette: fashionSelected.palette,
        silhouette: fashionSelected.silhouette, neckline: fashionSelected.neckline, items: fashionSelected.items.map((item) => `${item.slot}: ${item.name}`), shoppingKeywords: fashionSelected.shoppingKeywords,
        generationState: "selected", isRecommended: true, isSelected: true, image: null,
      });
    }
  ...
  if (source.salonBrief || snapshot.salonBrief.createdAt) {
    ...
    const services = brief?.details.services ?? { cut: snapshot.salonBrief.cut ? [snapshot.salonBrief.cut] : [], perm: [], color: snapshot.colorDecision.state === "confirmed" ? [snapshot.colorDecision.colorName] : [] };
    const design = brief ? [
  ...
  const tabs = CONSULTATION_REPORT_TAB_ORDER_V2.map((key) => ({ key, label: TAB_LABELS[key], sections: sections.filter((item) => item.tab === key) }))
    .filter((tab) => tab.key === "final" || tab.sections.length > 0);
  const filteredTabs = profile === "salon_handoff"
    ? tabs.map((tab) => ({ ...tab, sections: tab.sections.filter((item) => ["final-hair", "final-color", "salon-specification", "initial-care", "executive-summary"].includes(item.key)) })).filter((tab) => tab.key === "final" || tab.sections.length > 0)
    : tabs;
