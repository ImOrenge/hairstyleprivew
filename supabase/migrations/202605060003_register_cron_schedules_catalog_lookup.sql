-- Make register_app_cron_schedules pass plpgsql lint when pg_cron is absent.
-- The cron schema name is loaded from pg_namespace so dynamic SQL cannot be
-- pre-expanded by static analysis on projects without pg_cron enabled.
