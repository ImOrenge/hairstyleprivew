...
기존 기준점의 `운영 가격으로 확정하지 않는다`는 경계는 유지하되, 고객이 같은 공동 혜택 계약을 필요한 이용 기간으로 선택할 수 있도록 세 가격 가설을 랜딩에 `출시 예정가`로 노출한다.

| 상품 | 출시 예정가 | 기간 단위 | 랜딩 역할 |
|---|---:|---|---|
| Private Hair Direction | 99,000원 | 1회 | 공통 혜택을 한 번에 완결 |
| Total Image Direction | 189,000원 | 3개월 | 공동 계약과 결과 맥락을 3개월 유지 |
| Signature Style Membership | 649,000원 | 1년 | 공동 계약과 변화 이력을 1년 유지 |

- 공통 혜택은 사진·얼굴·추정 퍼스널 컬러 분석, 헤어 후보 9개와 패션 방향 9개 룩, 비교·결정·Salon Brief, Aftercare와 Style Dossier다.
- 세 플랜은 같은 공동 혜택 계약을 사용한다. 가격 차이는 기능 등급이 아니라 `1회 완결형`, `3개월 관리형`, `연간 관리형`의 계약 기간과 결과·이력 유지 범위로 설명한다.
- 계절·분기 기준의 정기 관리와 자동 업데이트 약속은 폐지한다. 퍼스널컬러의 사계절 분류는 분석 결과이며 상품 관리 주기를 뜻하지 않는다.
- 세 CTA는 모두 `/consulting/new`로 연결하며 결제를 시작하지 않는다.
- `189,000원`은 3개월 단위라는 사용자 정책 결정을 따른다. 자동갱신 여부나 제공 횟수는 별도 운영 계약 전까지 단정하지 않는다.
- 연간 Style Archive 등 미구현 범위는 출시 가설의 예정 산출물이며 현재 결제 가능한 완료 기능으로 주장하지 않는다.
