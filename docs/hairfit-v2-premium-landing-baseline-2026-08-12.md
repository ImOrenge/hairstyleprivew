...
기존 기준점의 `운영 가격으로 확정하지 않는다`는 경계는 유지하되, 고객이 같은 핵심 혜택을 필요한 관리 기간으로 선택할 수 있도록 세 가격 가설을 랜딩에 `출시 예정가`로 노출한다.

| 상품 | 출시 예정가 | 기간 단위 | 랜딩 역할 |
|---|---:|---|---|
| Private Hair Direction | 99,000원 | 1회 | 공통 혜택을 한 번에 완결 |
| Total Image Direction | 189,000원 | 3개월 | 같은 결과를 3개월 동안 관리·업데이트 |
| Signature Style Membership | 649,000원 | 1년 | 같은 결과와 변화 이력을 연간 누적 관리 |

- 공통 혜택은 사진·얼굴·추정 퍼스널 컬러 분석, 헤어 후보 9개와 패션 방향 9개 룩, 비교·결정·Salon Brief, Aftercare와 Style Dossier다.
- 플랜의 가격 차이는 기능 등급이 아니라 `1회 완결형`, `3개월 관리형`, `연간 관리형`의 관리 기간과 이력 축적 방식으로 설명한다.
