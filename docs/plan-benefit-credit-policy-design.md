...
| 헤어 생성 크레딧 | 기본 10크레딧 | `my-app/lib/pricing-plan.ts` |
| 패션 생성 크레딧 | 확정 헤어 기준 룩북 이미지 기본 20크레딧 | `my-app/lib/pricing-plan.ts` |
| 추가 에프터케어 프로그램 | 기본 30크레딧 | `my-app/lib/pricing-plan.ts` |
| 헤어 크레딧 차감 | 추천 보드 첫 결과 이미지 생성 시 `recommendation_grid_usage`로 1회 차감 | `my-app/app/api/generations/run/route.ts` |
| 패션 크레딧 차감 | 확정 헤어가 있는 패션 룩북 이미지 생성 시 `outfit_styling_usage`로 차감 | `my-app/app/api/styling/generate/route.ts` |
...
- 유료 플랜 패션 별도 상한을 제거하고, 잔여 크레딧과 확정 헤어 조건으로 제한한다.
...
6. `user_hair_records`, `user_aftercare_guides`, `user_care_contents` 6개를 생성한다.
7. 유료 생성이면 `consume_credits`로 30크레딧을 차감하고 `aftercare_program_usage` ledger를 남긴다.
8. 유료 차감 실패 시 새로 만든 `user_hair_records`를 삭제해 guide/content cascade cleanup을 유도한다.
9. 성공 응답에 `chargedCredits`, `firstAftercareProgramFreeUsed`, `careScheduledCount`를 포함한다.

운영 보강 기준:

- 현재 API 구현은 트랜잭션 대신 보상 삭제 방식이다.
- 동일 `generation_id` 재요청과 유료 차감 중복은 unique index로 막는다.
- 서로 다른 generation에서 첫 무료 에프터케어를 동시에 생성하는 경쟁 상태까지 완전히 막으려면 후속 RPC에서 사용자 row lock, 첫 무료 재확인, 생성 row insert, ledger insert를 하나의 트랜잭션으로 묶어야 한다.
- 첫 릴리스에서는 위 경쟁 상태를 잔여 리스크로 기록하고, 대량 트래픽 전에 RPC 전환을 우선 보강 항목으로 둔다.
...
```text
my-app/supabase/migrations/202607030001_plan_credit_policy_aftercare.sql
```

포함 내용:

- `idx_credit_ledger_unique_aftercare_program_usage` 추가
- `idx_user_hair_records_unique_generation_confirmation` 추가
- `generations.credits_used` DB default를 10으로 변경
- `consume_credits` 함수의 기본 `p_amount`를 10으로 재정의
- `care_generated_at` 갱신과 보상 삭제가 가능하도록 `user_hair_records` service_role update/delete 권한을 보강
...
- Standard 보관기간 365일
- Pro 보관기간 영구
- 패션 추천/생성 API가 확정 헤어 조건을 검사
- 에프터케어 API가 첫 무료와 추가 30크레딧 차감을 검사
- migration이 헤어 중복 확정과 에프터케어 중복 차감 방지 index를 포함
...
- 패션 룩북은 확정된 헤어스타일이 없거나 다른 variant를 요청하면 서버에서 차단된다.
