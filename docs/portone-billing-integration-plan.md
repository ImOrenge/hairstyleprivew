...
- `docs/portone-billing-migration-status.md`를 추가해 현재 linked Supabase project ref, migration history, `supabase db push --dry-run --workdir my-app` 결과를 기록했다. 2026-06-30에는 `202606290001`부터 `202606290005`까지, 2026-07-02에는 `20260702120012_payment_refund_requests.sql`까지 remote 적용 완료됐다.
...
- `npm run portone:migration:apply -- --write`로 linked Supabase project `dpzdhxlqnogfpubpslbf`에 기존 PortOne billing migration 배치를 적용했다. `202606290005`는 기존 RPC 반환형 변경 제한 때문에 `drop function if exists public.get_subscriptions_due_for_renewal(timestamptz)`를 추가한 뒤 재적용했다.
...
2026-07-02 로컬 브랜치 기준으로 앱 내부 환불 요청/승인 흐름을 시작했다. 기존에는 PortOne에서 이미 발생한 `Transaction.Cancelled`/`Transaction.PartialCancelled` 웹훅을 수신해 내부 거래, 구독, 크레딧 회수 상태를 반영하는 단계까지였고, 이번 환불 UX 구현은 아래 구조를 기준으로 진행한다.
...
8. 관리자 UI는 `/admin/refunds`에서 `provider_order_id`, 내부 결제 상태, 요청 금액, 요청 사유, 이전 환불 요청 상태를 보여준다. 전액 환불은 승인 액션으로 PortOne 취소 API를 호출하고, 부분환불은 정책 확정 전까지 `manual_review_required`로 전환한다.
...
| `P2. DB 원장 고정` | migration/RPC/idempotency | PortOne tracking 컬럼, 빌링키 암호화 컬럼, refund clawback, renewal retry, refund request migration 적용 확인 | `npm run portone:migration:check`, `npm run portone:db:smoke`, `npm run portone:db:smoke -- --write` | 원격 DB migration 6개 적용, Basic 갱신 9,900원/80크레딧, 지급/연장/회수 중복 호출 no-op, 환불 요청 원장 schema 확인 |
...
| `BILL-08-06` | `반영됨` | 테스트 DB migration/RPC smoke | Supabase migration, `npm run portone:migration:check`, `npm run portone:db:smoke`, `npm run portone:db:smoke -- --write` | 6개 migration이 적용되고 schema/RPC/idempotency smoke와 환불 요청 원장 schema 체크가 통과 |
