...
| 6 | admin latest | `GET /api/admin/hairstyles/cycles/latest` | active cycle, expiry, lineup count, next attempt가 반환된다. |
| 7 | not-due skip | `POST /api/admin/hairstyles/rebuild {"mode":"auto","onlyIfDue":true}` | TTL이 남아 있으면 `status:"skipped"`, `skipReason:"not_due"`를 반환한다. |
| 8 | dry-run | `POST /api/admin/hairstyles/rebuild {"mode":"auto","dryRun":true,"force":true}` | validation은 반환하지만 active pointer와 alert가 바뀌지 않는다. |
| 9 | forced rebuild | `POST /api/admin/hairstyles/rebuild {"mode":"auto","force":true}` | 새 cycle이 검증을 통과하면 active pointer가 교체된다. |
| 10 | recommendation smoke | 남성/여성 사용자 추천 생성 | 각 target에서 active cycle 기반 9개 lineup을 반환한다. |
| 11 | alert idempotency | 같은 active cycle로 rebuild 재호출 | `catalog_rotation` alert가 cycle당 1개만 존재한다. |
