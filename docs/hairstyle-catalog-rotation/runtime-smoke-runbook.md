...
| 2 | migration 적용 | `supabase db push --workdir my-app` | active pointer, lineup, event RPC, cron helper가 생성된다. |
| 3 | cron 등록 | `select public.register_hairstyle_catalog_rotation_cron('<web-url>', '<admin-secret>', '<edge-base-url>', '<service-role-key>');` | `cron.job`에 rotation check와 post-rotation mail job이 존재한다. |
| 4 | admin latest | `GET /api/admin/hairstyles/cycles/latest` | active cycle, expiry, lineup count, next attempt가 반환된다. |
| 5 | not-due skip | `POST /api/admin/hairstyles/rebuild {"mode":"auto","onlyIfDue":true}` | TTL이 남아 있으면 `status:"skipped"`, `skipReason:"not_due"`를 반환한다. |
| 6 | dry-run | `POST /api/admin/hairstyles/rebuild {"mode":"auto","dryRun":true,"force":true}` | validation은 반환하지만 active pointer와 alert가 바뀌지 않는다. |
| 7 | forced rebuild | `POST /api/admin/hairstyles/rebuild {"mode":"auto","force":true}` | 새 cycle이 검증을 통과하면 active pointer가 교체된다. |
| 8 | recommendation smoke | 남성/여성 사용자 추천 생성 | 각 target에서 active cycle 기반 9개 lineup을 반환한다. |
| 9 | alert idempotency | 같은 active cycle로 rebuild 재호출 | `catalog_rotation` alert가 cycle당 1개만 존재한다. |
| 10 | failure fallback | 강제 실패 조건에서 rebuild 호출 | failed cycle만 기록되고 기존 active cycle은 유지된다. |
| 11 | post-rotation mail | `cron-trend-emails-post-rotation` 실행 또는 함수 수동 호출 | due alert delivery가 중복 없이 기록된다. |
