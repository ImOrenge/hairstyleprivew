...
| readiness summary JSON | `--summaryJson=<path>` | `check`, `schemaVersion`, `generatedAt`, 최종 판정, 요청한 증거, remote readiness, `blockingMigrationDetails`, missing evidence, external blocker를 JSON 파일로 남긴다. |
...
| `npm run hairstyle:catalog:launch:check -- --allowMissingExternal --summaryJson=my-app/supabase/.temp/hairstyle-launch-summary.json` | 통과. 사람이 읽는 blocker 로그와 별도로 `check`, `schemaVersion`, `generatedAt`, `ok`, `requestedEvidence`, `checks`, `remoteReadiness.blockingMigrationDetails`, `missingEvidence`, `externalBlockers` JSON을 생성 |
