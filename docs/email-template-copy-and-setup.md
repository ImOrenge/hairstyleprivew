...
| 고객지원 답변 등록 | `[HairFit] 고객지원 답변이 등록되었습니다` | 답변 확인하기 | `my-app/app/api/admin/support/posts/[id]/route.ts` |
...
- 고객지원 답변 메일은 관리자 공식 답변이 처음 등록될 때만 발송되고, `admin_answer_email_sent_at`으로 중복 발송을 막는다.
