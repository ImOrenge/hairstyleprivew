...
| 구분 | 변수 |
| --- | --- |
| PortOne 공개 설정 | `NEXT_PUBLIC_PORTONE_V2_STORE_ID`, `NEXT_PUBLIC_PORTONE_V2_BILLING_KEY_CHANNEL_KEY`, `NEXT_PUBLIC_PORTONE_V2_PAYMENT_CHANNEL_KEY` |
| PortOne 서버 설정 | `PORTONE_V2_STORE_ID`, `PORTONE_V2_BILLING_KEY_CHANNEL_KEY`, `PORTONE_V2_PAYMENT_CHANNEL_KEY`, `PORTONE_V2_API_SECRET`, `PORTONE_V2_WEBHOOK_SECRET` |
...
V2 단건결제 배포 전에는 `--mode=v2-usage-pack`도 실행한다. 이 모드는 별도 V2 단건 채널키, V2 API/webhook secret, Supabase 설정, 그리고 공용 `/api/payments/webhook` 공개 HTTPS 경로를 확인한다.
...
npm run portone:env:check -- --mode=v2-usage-pack --webhookUrl=https://<your-domain>/api/payments/webhook
...
$env:CLOUDFLARE_API_TOKEN="<Cloudflare API token>"
$env:PORTONE_CLOUDFLARE_SECRET_SYNC_CONFIRM="hairstyleprivew"
npm run portone:cloudflare:secrets -- --write --verifyAfterWrite --only=PORTONE_V2_WEBHOOK_SECRET,PORTONE_V2_API_SECRET,BILLING_KEY_ENCRYPTION_SECRET,SUPABASE_SERVICE_ROLE_KEY,NEXT_PUBLIC_SUPABASE_URL,NEXT_PUBLIC_PORTONE_V2_STORE_ID,NEXT_PUBLIC_PORTONE_V2_BILLING_KEY_CHANNEL_KEY,NEXT_PUBLIC_PORTONE_V2_PAYMENT_CHANNEL_KEY
