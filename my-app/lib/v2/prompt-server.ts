...
function buildPromptInput(
  ...
  return {
    ...
    currentHair: {
      ...
      length: optionalText(currentHair.length),
      density: optionalText(currentHair.density),
      strandThickness: optionalText(currentHair.strandThickness),
      texture: optionalText(currentHair.texture),
      treatmentHistory: stringList(currentHair.treatmentHistory),
      damageLevel: optionalText(currentHair.damageLevel),
    },
    styleGoal: {
      imageKeywords: stringList(styleGoal.imageKeywords ?? snapshot.discovery.goals),
      ...
      changeLevel: enumText(
        styleGoal.changeLevel,
        ["subtle", "moderate", "bold", "unknown"] as const,
        "unknown",
      ),
      desiredServices: stringList(styleGoal.desiredServices ?? snapshot.discovery.desiredServices),
      notes: optionalText(styleGoal.notes ?? snapshot.discovery.notes),
    },
    maintenance: {
      morningMinutes: finiteNumber(maintenance.morningMinutes, 0, 240),
      heatStyling: enumText(
        maintenance.heatStyling,
        ["avoid", "sometimes", "comfortable", "unknown"] as const,
        "unknown",
      ),
      salonCycleWeeks: finiteNumber(maintenance.salonCycleWeeks, 1, 52),
