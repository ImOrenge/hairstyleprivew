...
import { assessMakeupSimulationQuality, deriveMakeupWorkspaceState, MAKEUP_RECIPE_TECHNIQUE_TOKENS, MAKEUP_RECIPE_PALETTE_ROLES, type MakeupSimulationInputV1, type MakeupSimulationInputV2, type MakeupSimulationOutputV1, type MakeupSimulationRunV1, type MakeupSimulationSelectionSnapshotV1 } from "@hairfit/shared/makeup";
...
import { readMakeupSourceImageDataUrl, resolveMakeupSourceImageReference } from "./makeup-source-image-server";
...
export async function queueMakeupSimulation(userId: string, consultationId: string) {
  ...
  const session = await getSupabaseAdminClient().from("consultation_sessions").select("snapshot").eq("id", consultationId).eq("user_id", userId).maybeSingle();
  ...
  const modules = snapshot.modules.map((item) => {
    const parameters = item.direction.technical.parameters;
    const paletteRole = typeof parameters.recipePaletteRole === "string" && MAKEUP_RECIPE_PALETTE_ROLES.includes(parameters.recipePaletteRole as never) ? parameters.recipePaletteRole as (typeof MAKEUP_RECIPE_PALETTE_ROLES)[number] : undefined;
    const techniqueTokens = Array.isArray(parameters.recipeTechniqueTokens) ? parameters.recipeTechniqueTokens.filter((token): token is (typeof MAKEUP_RECIPE_TECHNIQUE_TOKENS)[number] => typeof token === "string" && MAKEUP_RECIPE_TECHNIQUE_TOKENS.includes(token as never)) : undefined;
    return { module: item.module, enabled: item.state === "enabled" && item.direction.enabled, color: item.direction.colorFamily ?? "neutral", intensity: Math.round(item.direction.intensity * 100), finish: item.direction.texture ?? snapshot.context.finishPreference, reasonCodes: snapshot.rationale?.modules.find((reason) => reason.module === item.module)?.reasonCodes ?? [], ...(paletteRole ? { paletteRole } : {}), ...(techniqueTokens?.length ? { techniqueTokens } : {}) };
  });
  const source = await resolveMakeupSourceImageReference(userId, consultationId);
  const base = { consultationId, personalColor: { profileId: snapshot.source.personalColorProfileId, evidenceId: sessionSnapshot.personalColorDiagnosis?.evidenceId ?? null, palette: sessionSnapshot.personalColorDiagnosis?.stylingPalette ?? [], confidence: sessionSnapshot.personalColorDiagnosis?.qualityConfidence ?? null }, stylingContext: { hairSnapshotId: snapshot.source.selectedStyleId, colorSnapshotId: sessionSnapshot.colorDecision?.id ?? null, fashionDirectionId: null }, preserve: { identity: true as const, faceGeometry: true as const, hair: true as const, background: true as const, pose: true as const, lightingIntent: true as const }, prohibit: ["skin_shape_change", "face_slimming", "eye_enlargement", "nose_reshaping", "hair_restyle", "background_replacement", "beauty_retouching"] as const };
  const makeupBase = { interviewRevision: interview.profile.confirmedRevision ?? interview.profile.revision, selectedMode: snapshot.rationale.acceptedMode ?? snapshot.rationale.requestedMode, rationaleRevision: snapshot.rationale.revision, adjustmentDecision: snapshot.rationale.decision as "accept_adjustment" | "keep_selection", modules, exclusions: interview.profile.exclusions };
  const input: MakeupSimulationInputV1 | MakeupSimulationInputV2 = snapshot.recipeBinding ? {
    ...base,
    schemaVersion: "makeup-simulation-input-v2",
    sourceAsset: { id: source.assetId, fingerprint: source.fingerprint, kind: source.kind, storagePath: source.storagePath },
    makeup: { ...makeupBase, recipe: { id: snapshot.recipeBinding.recipeId, cycleId: snapshot.recipeBinding.cycleId, cycleVersion: snapshot.recipeBinding.cycleVersion, fingerprint: snapshot.recipeBinding.recipeFingerprint, presentationFamily: snapshot.recipeBinding.presentationFamily }, context: { occasions: snapshot.context.occasions, preparationMinutes: snapshot.context.preparationMinutes, skillLevel: snapshot.context.skillLevel, finishPreference: snapshot.context.finishPreference, facialHair: snapshot.context.facialHair } },
  } : { ...base, schemaVersion: "makeup-simulation-input-v1", sourceAsset: { id: source.assetId, fingerprint: source.fingerprint }, makeup: makeupBase };
...
export async function processMakeupSimulation(userId: string, consultationId: string, runId: string) {
  ...
  const input = row.input_snapshot as MakeupSimulationInputV1 | MakeupSimulationInputV2; const timestamp = new Date().toISOString();
  await db.from("makeup_simulation_runs_v2").update({ state: "generating", updated_at: timestamp }).eq("id", runId).eq("user_id", userId);
  try {
    const isV2 = input.schemaVersion === "makeup-simulation-input-v2";
    const imageDataUrl = await readMakeupSourceImageDataUrl(userId, consultationId, isV2 ? { kind: input.sourceAsset.kind, assetId: input.sourceAsset.id, storagePath: input.sourceAsset.storagePath, fingerprint: input.sourceAsset.fingerprint } : undefined);
    const result = await runMakeupSimulationCapability({ userId, consultationId, idempotencyKey: `${consultationId}:${input.sourceAsset.fingerprint}:${row.input_fingerprint}`, imageDataUrl, mode: input.makeup.selectedMode, palette: input.personalColor.palette, modules: input.makeup.modules, exclusions: input.makeup.exclusions, catalogCycleId: isV2 ? input.makeup.recipe.cycleId : null, recipeId: isV2 ? input.makeup.recipe.id : undefined, recipeFingerprint: isV2 ? input.makeup.recipe.fingerprint : undefined, presentationFamily: isV2 ? input.makeup.recipe.presentationFamily : undefined, occasion: isV2 ? input.makeup.context.occasions[0] : undefined, preparationMinutes: isV2 ? input.makeup.context.preparationMinutes : undefined, skillLevel: isV2 ? input.makeup.context.skillLevel : undefined, facialHair: isV2 ? input.makeup.context.facialHair : undefined });
  ...
  } catch (error) { await db.from("makeup_simulation_runs_v2").update({ state: "retry_required", error_code: error instanceof HairfitV2Error ? error.code : "MAKEUP_SIMULATION_GENERATION_FAILED", error_message: error instanceof Error ? error.message : "생성 실패", updated_at: new Date().toISOString() }).eq("id", runId).eq("user_id", userId); }
}
