...
import { ensureMakeupRoutine } from "../../../../../../lib/makeup/makeup-artifacts-server";
...
export async function POST(request: Request, { params }: Params) {
  ...
  const mode = body.mode === "full" ? "full" : body.mode === "compact" ? "compact" : undefined;
  const { sessionId } = await params;
  try { return NextResponse.json({ routine: await ensureMakeupRoutine(userId, sessionId, mode) }); }
