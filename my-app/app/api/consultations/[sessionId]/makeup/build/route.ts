...
import { buildMakeupDirection } from "../../../../../../lib/makeup/makeup-direction-server";
...
export async function POST(request: Request, { params }: Params) {
  ...
  if (!body || !Number.isInteger(body.expectedRevision)) return NextResponse.json({ error: "expectedRevision이 필요합니다." }, { status: 400 });
  const { sessionId } = await params;
  try { return NextResponse.json(await buildMakeupDirection(userId, sessionId, body.expectedRevision!)); }
