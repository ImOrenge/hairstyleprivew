  ...
  if (isFullStyleOfferingKey(offeringParam)) {
    ...
    return <AppPage className="grid gap-5 pb-16">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_380px]">
        ...
        <SurfaceCard as="aside" className="h-fit p-5"><p className="app-kicker">선택 상품</p><h2 className="mt-2 text-2xl font-black">{offer.koreanName}</h2><p className="mt-4"><strong className="text-3xl font-black">{offer.priceLabel}</strong><span className="text-sm text-[var(--app-muted)]"> {offer.periodLabel}</span></p><p className="mt-1 text-xs font-bold text-[var(--app-muted)]">부가세 포함 실제 승인 총액</p><ul className="mt-5 grid gap-2 text-sm leading-6">{PREMIUM_OFFER_POLICY.commonBenefits.map((benefit)=><li key={benefit}>· {benefit}</li>)}<li>· 상담당 새 3×3 전체 재시작 {offer.restartCount}회</li><li>· 상담당 D+30{offer.aftercareConsultationCount>1?"·60·90":""} AI 사후상담 {offer.aftercareConsultationCount}회</li><li>· 결과 {offer.retentionDays}일 보관</li><li>· 미사용 회차 이월 없음</li>{offer.autoRenew?<li>· {offer.periodLabel.replace("/ ","")} 단위 자동갱신</li>:null}</ul><p className="mt-5 border-t border-[var(--app-border)] pt-4 text-xs leading-5 text-[var(--app-muted)]">기간말 해지와 즉시 종료·환불 견적 요청을 제공합니다. 완전 미사용 계약만 자동 환불하며 사용 이력이 있으면 검토 후 안내합니다.</p></SurfaceCard>
      </div>
    </AppPage>;
  }
