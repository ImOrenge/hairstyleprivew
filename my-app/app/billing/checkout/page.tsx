...
export default async function BillingCheckoutPage({ searchParams }: BillingCheckoutPageProps) {
  ...
  if (isFullStyleOfferingKey(offeringParam)) {
    ...
    const requestedPriceVersion = Number(readSearchParam(params,"priceVersion"));
    const priceVersion = Number.isInteger(requestedPriceVersion) && requestedPriceVersion > 0
      ? requestedPriceVersion
      : PREMIUM_OFFER_POLICY.priceVersion;
    const returnPath = `/billing/checkout?offering=${encodeURIComponent(offeringParam)}&priceVersion=${priceVersion}${consultationId?`&consultationId=${encodeURIComponent(consultationId)}`:""}`;
    ...
    return <AppPage className="grid gap-5 pb-16">
      ...
      <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_380px]">
        <Panel as="section" className="p-5 sm:p-6"><FullStyleCheckoutForm offeringKey={offeringParam} priceVersion={priceVersion} consultationId={consultationId||undefined} initialBuyerName={user?.fullName||user?.firstName||""} initialBuyerEmail={user?.primaryEmailAddress?.emailAddress||""} initialBuyerPhone={user?.primaryPhoneNumber?.phoneNumber||""} /></Panel>
