...
export default async function ConsultingArchivePage() {
  ...
  return <AppPage className="grid gap-5 pb-16">
    <Panel as="header" className="p-5 sm:p-7"><p className="app-kicker">Annual Style Archive</p><h1 className="mt-2 text-3xl font-black tracking-tight sm:text-4xl">연간 스타일 아카이브</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-[var(--app-muted)]">연간 플랜으로 완료한 퍼스널 컬러와 헤어·염색·메이크업·패션 결정을 회차별로 모아 비교합니다. 각 회차에는 30·60·90일 체크인이 제공되고 결과는 완료일부터 365일 보관됩니다.</p></Panel>
    {!hasAnnualArchive?<Panel as="section" className="p-5 sm:p-6"><h2 className="text-xl font-black">연간 플랜에서 제공됩니다</h2><p className="mt-2 text-sm text-[var(--app-muted)]">연 4회 결과와 변화 기록, 연간 종합 리포트를 한곳에 보관할 수 있습니다.</p><Link href="/consulting/plans" className="f-landing-cta mt-4 inline-flex">연간 플랜 확인</Link></Panel>:null}
