...
export function SelectedVariantCard({ variant, analysis, generationId }: SelectedVariantCardProps) {
  ...
  return (
    <section className="h-full rounded-xl border border-stone-200 bg-white p-5 shadow-[0_18px_50px_-36px_rgba(15,23,42,0.35)]">
      <p className="text-xs font-bold uppercase tracking-[0.16em] text-stone-400">Selected hairstyle</p>
      <h2 className="mt-2 text-2xl font-black leading-tight text-stone-900">
        {variant?.label || "Selected style"}
      </h2>

      <div className="mt-4 space-y-2">
        <p className="text-base leading-7 text-stone-700">
          {translate(variant?.reason) || "Select a completed hairstyle to review the detailed style rationale."}
        </p>
      ...
      {analysis?.summary ? (
        <div className="mt-5 rounded-lg bg-stone-50 p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.14em] text-stone-400">Recommendation basis</p>
