...
type Completion = { paymentId?:string;contractDocumentStatus?:"pending"|"sent"|"delivery_uncertain"|"failed";error?:string };
...
export function FullStyleCheckoutForm({ offeringKey,priceVersion,consultationId,initialBuyerName="",initialBuyerEmail="",initialBuyerPhone="" }:{
  ...
  const completeAttempt=async(checkoutAttemptId:string)=>{
    const response=await fetch("/api/payments/full-style/complete",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({checkoutAttemptId})});
    const result=await response.json().catch(()=>({})) as Completion;
    if(!response.ok)throw new Error(result.error||"결제를 확인하지 못했습니다.");
    if(result.contractDocumentStatus&&result.contractDocumentStatus!=="sent"){
      setPendingCompletionId(checkoutAttemptId);
      throw new Error("결제는 완료됐지만 계약 문서 이메일 제공을 확인하지 못했습니다. 다시 보내기를 눌러 결제 없이 재시도해 주세요.");
    }
    setPendingCompletionId(null);finish(result);
  };
  const submit=async()=>{
    if(!agreed||!refundAgreed){setError("결제 조건과 청약철회·환불 정책을 모두 확인해 주세요.");return;} setPending(true);setError(null);
    try{
      if(pendingCompletionId){await completeAttempt(pendingCompletionId);return;}
      ...
      if(prepared.purchaseMode==="one_time"){
        ...
        await completeAttempt(prepared.checkoutAttemptId);return;
      }
      ...
      const subscribe=await fetch("/api/payments/full-style/subscribe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({checkoutAttemptId:prepared.checkoutAttemptId,billingKey:issued.billingKey,billingIssueToken:issued.billingIssueToken})});const result=await subscribe.json().catch(()=>({})) as Completion;if(!subscribe.ok)throw new Error(result.error||"정기결제를 시작하지 못했습니다.");if(result.contractDocumentStatus&&result.contractDocumentStatus!=="sent"){setPendingCompletionId(prepared.checkoutAttemptId);throw new Error("결제는 완료됐지만 계약 문서 이메일 제공을 확인하지 못했습니다. 다시 보내기를 눌러 결제 없이 재시도해 주세요.");}finish(result);
  ...
  return <form className="grid gap-5" onSubmit={(event)=>{event.preventDefault();void submit();}}>
    ...
    <Button type="submit" disabled={pending||!agreed||!refundAgreed} loading={pending}>{pendingCompletionId?"결제 없이 계약 문서 다시 받기":"주문 확인 후 결제하기"}</Button>
  </form>;
}
