...
export function DirectionWorkbench({ snapshot, mutate, saving }: { snapshot: ConsultationSnapshot; mutate: (patch: Omit<ConsultationPatch, "expectedVersion">) => Promise<unknown>; saving: boolean }) {
  ...
  return <WorkbenchGrid input={
    <Panel className="grid gap-6 p-5 sm:p-7">
      ...
      {pendingAdjustment.length ? <SurfaceCard className="p-4" data-hair-adjustment-intent="pending"><p className="app-kicker">Requested adjustment</p><ul className="mt-2 grid gap-1 text-sm leading-6">{pendingAdjustment.map((item, index) => <li key={`${item.aspect}-${index}`}><strong>{item.aspect}</strong> · {item.value}</li>)}</ul><p className="mt-2 text-xs text-[var(--app-muted)]">아래 생성 축에 반영해 확정하면 이전 9개를 덮어쓰지 않고 새 9개 revision을 만듭니다.</p></SurfaceCard> : null}
