...
export function PhotoWorkbench({ snapshot, mutate, saving }: {
  ...
  const selectFile = async (selected: File) => {
    ...
    try {
      ...
      setPhoto((current) => ({
        ...
        uploadedAt: null,
        expiresAt: null,
        quality: current.quality.map((item) => ({ ...item, status: "pending", message: "AI 분석 대기" })),
      }));
    } catch (cause) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/consultations/${encodeURIComponent(snapshot.sessionId)}/photo-analysis`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ draftId: receipt.draftId, expectedVersion: snapshot.version }),
      });
  ...
  const reset = () => {
    ...
    setPreviewUrl(null);
    setFile(null);
    setPhoto({ ...snapshot.photo, draftId: null, clientRequestId: null, uploadedAt: null, expiresAt: null });
    resetValidation();
    setError(null);
  ...
  return <WorkbenchGrid>
    <Panel className="grid gap-6 p-5 sm:p-7">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--app-muted)]">이 화면에서 private Storage 업로드와 AI 분석을 진행합니다. 구 마법사로 이동하지 않습니다.</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2"><SaveStageButton loading={working || saving} disabled={!file && !photo.draftId} onClick={() => void analyze()}>사진 업로드 및 AI 분석</SaveStageButton><Button type="button" variant="ghost" disabled={working || saving} onClick={reset}>다시 선택</Button></div>
    </Panel>
