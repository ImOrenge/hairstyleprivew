...
export function DecisionWorkbench({ snapshot, mutate, saving }: { snapshot: ConsultationSnapshot; mutate: (patch: Omit<ConsultationPatch, "expectedVersion">, options?: { navigate?: boolean }) => Promise<unknown>; saving: boolean }) {
  ...
  const createAutomaticBrief = async () => {
    ...
    const data = (await response.json().catch(() => ({}))) as {
      brief?: { version?: number; summary?: string; cut?: unknown; volumeTexture?: unknown; styling?: string[]; cautions?: string[]; createdAt?: string };
      error?: string;
    };
  ...
  const saveDecision = async () => {
    ...
    try {
      ...
      taskRuntime.updateTask({ phaseKey: "services", phaseIndex: 1, completedUnits: 1, detail: "확정된 선택과 필요한 시술 항목을 연결했습니다." });
  ...
  return <WorkbenchGrid input={
    ...
    <ConsultationSystemData snapshot={snapshot} items={[
      ...
      { label: "Required services", value: decision.services.join(" · ") || "커트·드라이 중심" },
    ]} />
  </>} />;
}
