...
export function ReportReceipt({ report }: { report: ConsultationReportViewModelV1 }) {
  return <article data-report-receipt="true" className="f-consulting-report__receipt mx-auto w-full max-w-[52rem] border border-[var(--app-border)] bg-[var(--app-surface)] text-[var(--app-text)]">
    <header className="f-consulting-report__header grid gap-7 border-b border-[var(--app-border)] p-5 sm:p-8">
      ...
      {report.heroImage?.src ? <figure data-report-keep="true" className="grid gap-3"><div className="mx-auto aspect-[4/5] max-h-[38rem] w-full"><img src={report.heroImage.src} alt={report.heroImage.alt} decoding="async" loading="eager" className="h-full w-full object-contain" /></div><figcaption className="text-xs font-bold text-[var(--app-muted)]">{report.heroImage.label}</figcaption></figure> : null}
    ...
    <div>
      {report.sections.map((item) => <section key={item.key} id={`report-${item.key}`} data-report-section="true" className="f-consulting-report__section border-b border-[var(--app-border)] p-5 last:border-b-0 sm:p-8">
        ...
        {item.images.length ? <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{item.images.map((image) => <figure key={image.id} data-report-keep="true" className="grid gap-2 border border-[var(--app-border)] p-2">{image.src ? <div className="aspect-[4/5]"><img src={image.src} alt={image.alt} decoding="async" loading="lazy" className="h-full w-full object-cover" /></div> : <div className="grid aspect-[4/5] place-items-center bg-[var(--app-surface-muted)] text-xs font-bold">{image.status === "failed" ? "생성 실패" : "이미지 준비 중"}</div>}<figcaption className="text-xs font-bold">{image.label}</figcaption></figure>)}</div> : null}
