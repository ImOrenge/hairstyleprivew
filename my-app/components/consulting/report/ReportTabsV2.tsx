...
export function ReportTabsV2({ tabs, defaultTab, narrative, onRetryNarrative, onRetryMakeupReport }: { tabs: ConsultationReportTabV2[]; defaultTab: ConsultationReportTabKeyV2; narrative?: ConsultationReportNarrativeEnvelopeV1; onRetryNarrative?: () => void; onRetryMakeupReport?: () => void }) {
  ...
  return (
    <div className="f-consulting-report-v2__tabs">
      <div role="tablist" aria-label="컨설팅 결과 영역" className="f-consulting-report-v2__tablist" data-report-screen-only="true">
        {tabs.map((tab, index) => (
          <button
            ...
            className="f-consulting-report-v2__tab"
          >
            <span>{tab.label}</span>
            <span aria-label={`${tab.sections.length}개 결과`} className="f-consulting-report-v2__tab-count">
              {tab.sections.length}
            </span>
          </button>
        ))}
      ...
      {tabs.map((tab) => {
        ...
        return (
          <section key={tab.key} role="tabpanel" id={`report-panel-${tab.key}`} aria-labelledby={`report-tab-${tab.key}`} aria-hidden={activeTab !== tab.key} data-active={activeTab === tab.key ? "true" : "false"} data-report-tab-panel={tab.key} className="f-consulting-report-v2__panel">
            <header className="f-consulting-report-v2__group-heading border-b border-[var(--app-border)] px-5 py-4 sm:px-8">
              <p className="app-kicker">상담 결과</p>
              <h2 className="mt-1 text-2xl font-black">{tab.label}</h2>
            </header>
            ...
            {tab.sections.map((section) => (
              <ReportSectionV2 key={section.key} section={section} explanationCovered={Boolean(narrative) && section.key === "executive-summary"} onRetryMakeupReport={onRetryMakeupReport} />
            ))}
          </section>
        );
      })}
