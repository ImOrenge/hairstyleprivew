    ...
    <WorkbenchGrid input={<div className="grid gap-5">
      <Panel className="grid gap-5 p-5 sm:p-7">
        ...
        <DefinitionRows items={[
          ...
          { label: "다음 행동", value: viewState === "primary-review" ? "AI 추천을 확인하고 그대로 진행하거나 조정을 요청해 주세요." : "준비가 끝나면 가장 잘 맞는 스타일부터 보여드릴게요." },
        ]} />
      ...
      <Panel className="overflow-hidden">
        ...
        <div className="grid gap-4 p-5"><div><p className="app-kicker">가장 잘 맞는 추천</p><h2 className="mt-2 text-2xl font-black">{primarySnapshot?.label ?? (primaryVariant ? "AI 추천 스타일" : "평가 중")}</h2><p className="mt-2 text-sm leading-6 text-[var(--app-muted)]">{primarySnapshot?.reason ?? "준비와 품질 확인이 끝나면 가장 적합한 한 가지를 이유와 함께 설명합니다."}</p></div>
      ...
      <SurfaceCard className="p-5" data-hair-generated-gallery="all-nine">
        <div><p className="app-kicker">비교할 수 있는 헤어</p><h2 className="mt-2 text-xl font-black">준비된 스타일 {acceptedCount} / 9</h2><p className="mt-2 text-sm leading-6 text-[var(--app-muted)]">가장 잘 맞는 추천을 먼저 표시하고, 다른 스타일도 빠짐없이 비교할 수 있게 보여드립니다.</p></div>
        <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">{recommendation.board?.variants.map((variant, index) => {
          ...
          return <article key={variant.id} className={`overflow-hidden border ${isPrimary ? "border-[var(--app-border-strong)] ring-2 ring-[var(--app-ring)]" : "border-[var(--app-border)]"}`} data-hair-slot={variant.slot} data-hair-slot-state={accepted ? "accepted" : variant.status}>
            ...
            <div className="p-3"><div className="flex items-center justify-between gap-2"><strong>비교 스타일 {index + 1}</strong>{isPrimary ? <span className="text-xs font-black text-[var(--app-accent-strong)]">AI 추천</span> : null}</div><p className="mt-1 text-xs text-[var(--app-muted)]">얼굴·모발·관리 조건을 반영한 비교안</p></div>
          </article>;
        }) ?? Array.from({ length: 9 }, (_, index) => <div key={index} className="grid aspect-[4/5] place-items-center border border-[var(--app-border)] bg-[var(--app-surface-muted)] text-xs text-[var(--app-muted)]">비교 스타일 {index + 1} · 준비 중</div>)}</div>
      </SurfaceCard>
    </div>} />
  </div>;
}
