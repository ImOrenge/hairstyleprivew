...
export function FloatingStageControls({ snapshot, stage, onOpenMap, chapterNavigationEnabled = true }: { snapshot: ConsultationSnapshot; stage: ConsultationStage; onOpenMap: () => void; chapterNavigationEnabled?: boolean }) {
  ...
  return (
    <nav className="fixed inset-x-0 bottom-4 z-40 mx-auto flex w-fit max-w-[calc(100%-1rem)] items-center gap-2 bg-[color-mix(in_srgb,var(--app-bg)_88%,transparent)] p-1 backdrop-blur" aria-label="상담 단계 이동">
      {!chapterNavigationEnabled && previousAllowed ? <Link href={consultationStageHrefForPath(snapshot.sessionId, previousAllowed, pathname)} className={control} aria-label="이전 상담 화면"><ArrowLeft className="h-4 w-4" aria-hidden /><span className="hidden sm:inline">Previous</span></Link> : !chapterNavigationEnabled ? <span className={`${control} opacity-40`} aria-disabled="true"><ArrowLeft className="h-4 w-4" aria-hidden /></span> : null}
      <button type="button" onClick={onOpenMap} className={control}><List className="h-4 w-4" aria-hidden />{chapterNavigationEnabled ? "Chapters" : "All stages"}</button>
      {recommended !== stage && snapshot.journey.allowedStages.includes(recommended) ? <Link href={consultationStageHrefForPath(snapshot.sessionId, recommended, pathname)} className={control} aria-label={`AI 추천 작업: ${presentation.recommendedTask.label}`}><Sparkles className="h-4 w-4" aria-hidden /><span className="hidden sm:inline">{chapterNavigationEnabled ? presentation.recommendedTask.label : recommendedDefinition?.task ?? recommended}</span></Link> : null}
