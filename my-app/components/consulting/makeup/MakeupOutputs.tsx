...
export function MakeupOutputs({ sessionId, routine, brief, onRefresh }: { sessionId: string; routine: MakeupRoutine | null; brief: MakeupArtistBrief | null; onRefresh: () => Promise<void> }) {
  ...
  return <section className="grid gap-5 lg:grid-cols-2">
    ...
    <div className="grid content-start gap-5"><SurfaceCard className="p-5"><p className="app-kicker">아티스트에게 보여줄 내용</p><h2 className="mt-2 text-lg font-black">확정한 메이크업 방향</h2><p className="mt-2 text-sm text-[var(--app-muted)]">퍼스널 컬러와 확정 헤어에 맞춘 내용을 그대로 전달합니다.</p><dl className="mt-4 grid gap-3 text-sm"><div><dt className="text-[var(--app-muted)]">사용할 부위</dt><dd className="font-bold">{brief.moduleSummaries.filter((item) => item.enabled).map((item) => LABELS[item.module]).join(" · ")}</dd></div></dl></SurfaceCard><SurfaceCard className="p-5"><p className="app-kicker">제품 찾기</p><h2 className="mt-2 text-lg font-black">브랜드와 관계없이 활용할 검색어</h2><div className="mt-4 flex flex-wrap gap-2">{productTerms.map((term) => <span key={term} className="border border-[var(--app-border)] px-2 py-1 text-xs font-bold">{term}</span>)}</div></SurfaceCard><SurfaceCard className="p-5"><p className="app-kicker">공유 범위 선택</p><h2 className="mt-2 text-lg font-black">아티스트에게 전달</h2><label className="mt-4 flex min-h-11 items-start gap-3 text-sm font-bold"><input type="checkbox" className="mt-1" checked={includeSourcePhoto} onChange={(event) => setIncludeSourcePhoto(event.target.checked)} />원본 사진도 명시적으로 포함</label><p className="text-xs text-[var(--app-muted)]">기본값은 포함하지 않음입니다. 사진을 포함하면 보존 만료 시점까지만 링크가 유지됩니다.</p>{share ? <div className="mt-4 border border-[var(--app-border)] p-3 text-sm"><a href={share.url} className="break-all underline" target="_blank" rel="noreferrer">{share.url}</a><p className="mt-2 text-xs">사진 {share.sourcePhotoIncluded ? "포함" : "미포함"} · {new Date(share.expiresAt).toLocaleString("ko-KR")} 만료</p><div className="mt-3"><Button type="button" variant="secondary" loading={working} onClick={() => void revokeShare()}>공유 취소</Button></div></div> : <div className="mt-4"><Button type="button" loading={working} onClick={() => void createShare()}>7일 공유 링크 만들기</Button></div>}{error ? <p role="alert" className="mt-3 text-sm text-red-400">{error}</p> : null}</SurfaceCard></div>
  </section>;
}
