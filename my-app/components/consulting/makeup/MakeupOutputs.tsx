...
export function MakeupOutputs({ sessionId, routine, brief, onRefresh }: { sessionId: string; routine: MakeupRoutine | null; brief: MakeupArtistBrief | null; onRefresh: () => Promise<void> }) {
  ...
  return <details className="border border-[var(--app-border)] bg-[var(--app-surface)] p-4 sm:p-5" data-makeup-secondary-actions="true">
    <summary className="cursor-pointer font-black">제품 찾기와 아티스트 공유</summary>
    <p className="mt-2 text-sm text-[var(--app-muted)]">필요할 때 제품 검색어를 확인하거나 확정 리포트를 안전하게 공유할 수 있습니다.</p>
