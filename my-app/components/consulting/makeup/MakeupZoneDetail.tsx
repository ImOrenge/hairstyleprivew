...
export function MakeupZoneDetail({ active, revision, disabled, onPatch }: { active: MakeupModuleDirection; revision: number; disabled: boolean; onPatch: (patch: Record<string, unknown>) => Promise<void> | void }) {
  ...
  return <SurfaceCard className="p-5">
    ...
    <div className="mt-2 flex flex-wrap items-center justify-between gap-2"><h3 className="text-lg font-black">{LABELS[active.module]}</h3><span className="border border-[var(--app-border)] px-2 py-1 text-xs font-black">저장됨</span></div>
    ...
    <p className="mt-4 text-xs text-[var(--app-muted)]">퍼스널 컬러 분석과 연결한 메이크업 방향입니다.</p>
  </SurfaceCard>;
}
