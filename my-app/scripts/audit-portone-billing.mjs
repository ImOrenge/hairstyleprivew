...
assertAbsent(pricingPlan, 'PRICING_CREDITS_PER_STYLE|PRICING_CREDITS_PER_OUTFIT|PRICING_CREDITS_PER_AFTERCARE_PROGRAM', "Displayed and executed paid-action prices must not drift through environment overrides");
assertAbsent(envExample, 'PRICING_CREDITS_PER_STYLE|PRICING_CREDITS_PER_OUTFIT|PRICING_CREDITS_PER_AFTERCARE_PROGRAM', "Environment examples must not advertise ignored paid-action unit-price overrides");
assertIncludes(planBenefitDisplay, 'servicePasses = getServicePassCounts\\(plan\\.credits\\)', "Plan display must use the shared service-pass conversion");
assertIncludes(planBenefitDisplay, 'hairFashionSetCount: servicePasses\\.fashionSetCount', "Plan display must include the hair generation required by each fashion set");
...
assertIncludes(pricingEn, '"pricing\\.usage\\.aftercarePolicy": "\\{\\{count\\}\\}-use care pass · first program free once per account', "English pricing copy must show paid care passes separately");
...
assertIncludes(billingCheckoutPage, '헤어 이용권: \\{plan\\.usage\\.hairOnlyCount', "Checkout must show hair pass counts");
assertIncludes(billingCheckoutPage, '패션 이용권: \\{plan\\.usage\\.hairFashionSetCount[\\s\\S]*?헤어\\+패션 세트 기준', "Checkout must show fashion set counts and composition");
assertIncludes(billingCheckoutPage, '케어 이용권: \\{plan\\.usage\\.aftercareProgramCount[\\s\\S]*?최초 1회 계정당 무료', "Checkout must separate paid care passes from the first-free benefit");
assertAbsent(billingCheckoutPage, '서비스 이용량|이용량|차감|plan\\.credits', "Checkout must not expose internal balance terminology or totals");
assertIncludes(usagePack, 'label: `추가 이용권 \\${input\\.size}`', "Usage packs must use size names instead of internal totals");
assertIncludes(usagePack, 'serviceLabel = `헤어 \\${servicePasses\\.hairCount}회·패션 \\${servicePasses\\.fashionSetCount}세트·케어 \\${servicePasses\\.careCount}회`', "Usage pack order names must contain service-specific pass counts");
assertIncludes(billingUsagePage, '서비스별 최대: \\{formatServicePassCountsKo\\(item\\.servicePasses\\)\\}', "One-time pass cards must show service-specific maximums");
assertIncludes(billingUsagePage, '패션은 헤어\\+패션 1세트 기준', "One-time pass cards must explain fashion set composition");
assertIncludes(billingUsagePage, '최초 케어 무료를 사용하지 않은 계정은 1회 무료 혜택을 별도로 이용할 수 있습니다', "One-time pass checkout must separate the account-level free care use");
assertAbsent(billingUsagePage, '이용량|차감|credits\\.toLocaleString|pack\\.credits', "One-time pass checkout must not expose internal balance terminology or totals");
...
assertIncludes(generationAcceptRoute, 'p_generated_assets_expires_at:\\s*generatedAssetsExpiresAt', "durable generation acceptance must persist the plan retention deadline");
...
assertIncludes(paidActionAtomicMigration, "'outfit_styling_failure_refund'", "Styler migration must refund a failed reserved execution");
