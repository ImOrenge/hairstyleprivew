...
assertIncludes(pricingKo, '패션 룩북 이미지는 확정 헤어 기준 \\{\\{outfitCredits\\}\\}크레딧', "Korean credit note must state fashion depends on confirmed hair");
...
assertIncludes(pricingEn, 'Fashion lookbook images cost \\{\\{outfitCredits\\}\\} credits from a confirmed hair style', "English credit note must state fashion depends on confirmed hair");
...
assertIncludes(billingCheckoutPage, '패션 룩북 이미지 생성: 확정 헤어 기준 \\{plan\\.creditsPerOutfit\\.toLocaleString\\("ko-KR"\\)\\}크레딧', "Checkout must show confirmed-hair fashion cost");
