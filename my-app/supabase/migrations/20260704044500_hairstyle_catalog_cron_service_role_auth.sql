-- Allow hairstyle catalog rotation cron registration to use service-role
-- authentication for the web admin API when INTERNAL_API_SECRET is unavailable.
...
begin
  ...
  v_rotation_headers := jsonb_build_object(
    'Content-Type', 'application/json',
    'Authorization', 'Bearer ' || p_service_role_key,
    'apikey', p_service_role_key
  );

  if nullif(p_admin_secret, '') is not null then
    v_rotation_headers := v_rotation_headers || jsonb_build_object('x-admin-secret', p_admin_secret);
  end if;
